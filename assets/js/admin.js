jQuery(document).ready(function ($) {
    ...
    // Simple form submission handler for sources
    $(document).on('submit', '#ng-source-form', function(e) {
        e.preventDefault();
        console.log('Source form submitted'); // Debug
        console.log('Form element:', $(this)); // Debug
        console.log('Nonce field exists:', $('#ng-source-form input[name="nonce"]').length > 0); // Debug
        console.log('Nonce field value:', $('#ng-source-form input[name="nonce"]').val()); // Debug
        ...
        // Get form data
        var formData = {
            action: 'ng_save_source',
            nonce: $('#ng-source-form input[name="nonce"]').val(), // Use nonce from form
            id: $('#ng-source-id').val(),
            source_type: $('#ng-source-type').val(),
            source_name: $('#ng-source-name').val(),
            sync_interval_minutes: $('#ng-source-interval').val()
        };
        
        // Add URL based on source type
        if ($('#ng-source-type').val() === 'website') {
            formData.website_url = $('#ng-website-url').val();
        } else if ($('#ng-source-type').val() === 'rss') {
            formData.rss_feed = $('#ng-rss-feed').val();
        }
        
        console.log('Form data:', formData); // Debug
        console.log('Nonce from form:', $('#ng-source-form input[name="nonce"]').val()); // Debug
        
        // Show loading state
        $('#ng-source-form button[type="submit"]').prop('disabled', true).text('Saving...');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // Reset button
                $('#ng-source-form button[type="submit"]').prop('disabled', false).text('Save Source');
    ...
    function validateApiKeyRealtime() {
        ...
        function validateKey() {
            ...
            console.log('Real-time validation triggered'); // Debug
            console.log('API Key:', apiKey.substring(0, 10) + '...'); // Debug
            ...
            if (apiKey.indexOf('••••') === 0 || apiKey === '') {
                console.log('Skipping validation - masked or empty'); // Debug
            ...
            $.ajax({
                ...
                beforeSend: function(xhr) {
                    ...
                    console.log('Data:', {
                        ...
                        provider: provider,
                        nonce: nomadsguruParams.nonce ? 'present' : 'missing'
                    });
                },
                success: function(response) {
                    console.log('AJAX response:', response); // Debug
                ...
                error: function(xhr, status, error) {
                    ...
                    $validationResult.removeClass('ng-validating ng-valid')
                                      .addClass('ng-invalid')
                                      .html('<span class="ng-validation-icon">✗</span> Validation failed: ' + error);
                }
            });
        }
    ...
    $(document).on('click', '#manual_validate_key', function(e) {
        e.preventDefault();
        console.log('Manual validation triggered'); // Debug
        var $apiKeyField = $('#api_key');
        ...
        if (apiKey.indexOf('••••') === 0) {
            alert('Please enter a new API key to validate');
            return;
        }
