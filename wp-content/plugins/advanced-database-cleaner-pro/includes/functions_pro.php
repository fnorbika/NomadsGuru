...
function aDBc_filter_results_in_all_items_array(&$aDBc_all_items, $aDBc_tables_name_to_optimize, $aDBc_tables_name_to_repair){
	...
	// Prepare an array with values of autoload that should be autoloaded
	$autoload_values = array("yes"); // Default value is "yes" prior to WP 6.6.0
	...
	if(function_exists('is_multisite') && is_multisite()){
		...
		if(!empty($_GET['autoload']) && $_GET['autoload'] != "all"){
			...
			if ($autoload_param == "yes") {

				foreach($aDBc_all_items as $item_name => $item_info){
					foreach($item_info['sites'] as $site_id => $site_item_info){
						if (!in_array($site_item_info['autoload'], $autoload_values)){
							unset($aDBc_all_items[$item_name]['sites'][$site_id]);
						}
					}
				}

			} else if ($autoload_param == "no") {
		...
		foreach($aDBc_all_items as $item_name => $item_info){
			...
			if($filter_on_autoload){
				...
				if ($autoload_param == "yes") {
					...
					if (!in_array($item_info['sites'][1]['autoload'], $autoload_values))
						array_push($names_to_delete, $item_name);
...
function aDBc_new_run_search_for_items(){
	...
	// set the max execution time for the scan process to 60 seconds if needed
	aDBc_set_max_execution_time_if_needed();
	...
	***********************************************************************************************************************/
	if ( $aDBc_iteration == 0) {
		...
		if(empty($aDBc_items_to_scan)){
			...
			// In class_clean_cron we don't use "|" to separate items, we use a json format. So we need to decode it.
			if ($items_type == "tasks") {

				foreach($aDBc_items_to_scan as $item_to_scan){

					$json = wp_unslash( $item_to_scan ); // Unslash because WP adds slashes to the POST data that is sent
					$data = json_decode( $json, true );
					$item_name = trim($data['hook_name']);
				...
				// For tables and options, we split the string by "|" as usual
				foreach($aDBc_items_to_scan as $item_to_scan){
					$columns = explode("|", trim($item_to_scan), 2);
					$item_name = wp_unslash(trim($columns[1])); // Unslash because WP adds slashes to the POST data that is sent
					if(!array_key_exists($item_name, $items_to_search_for)){
						$items_to_search_for[$item_name] = array('belongs_to' => '', 'maybe_belongs_to' => '');
					}
				}
...
function aDBc_shutdown_due_to_timeout(){
	...
	aDBc_restore_max_execution_time();
}
...
function aDBc_refresh_and_create_php_files_paths($items_type){
	...
	foreach($folders_to_scan as $folder_to_scan){
		...
		// Reset the last collected file path to empty for the next folder
		$last_collected_file_path = '';
    ...
    foreach ($entries as $entry) {
        ...
        if ($should_start_processing && ($found_resume_file || $resume_file === '')) {
            if (is_file($full_path)) {
				// Check if the file is a PHP file and its size is less than 2MB
				if(pathinfo($full_path, PATHINFO_EXTENSION) === 'php' && filesize($full_path) < 2 * 1024 * 1024){
...
/*************************************************************************************************************
 * Set the max execution time for the scan process to 60 seconds.
 * ***********************************************************************************************************/
function aDBc_set_max_execution_time_if_needed() {

	if ( ! function_exists( 'ini_get' ) )
		return;

	global $adbc_original_time_limit;
	$adbc_original_time_limit = ini_get( 'max_execution_time' );

	if ( ! $adbc_original_time_limit || $adbc_original_time_limit <= 60 )
		return;
...
/*************************************************************************************************************
 * Restore the max execution time to the original value.
 * ***********************************************************************************************************/
function aDBc_restore_max_execution_time() {

	global $adbc_original_time_limit;

	if ( ! $adbc_original_time_limit )
		return;

	if ( function_exists( 'set_time_limit' ) ) {
		@set_time_limit( $adbc_original_time_limit );
		return;
	}

	if ( function_exists( 'ini_set' ) )
		@ini_set( 'max_execution_time', $adbc_original_time_limit );

}

/*************************************************************************************************************
 * This function hides the "Not categorized yet" message by adding a setting in the database
 * ***********************************************************************************************************/
function aDBc_hide_not_categorized_yet_msg(){
	...
	$aDBc_settings['hide_not_categorized_yet_msg'] = "yes";
